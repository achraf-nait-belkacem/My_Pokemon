...
from backend.data_manager import load_pokedex, load_all_ennemi, save_team
...
class First_screen:
    def __init__(self, screen):
        ...
        self.pokemons = load_pokedex()
        self.ennemi = load_all_ennemi()
    ...
    def handle_events(self):
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if self.state == "MENU":
                    if event.key == pygame.K_UP: self.selected_index = (self.selected_index - 1) % self.buttons_count
                    elif event.key == pygame.K_DOWN: self.selected_index = (self.selected_index + 1) % self.buttons_count
                    elif event.key == pygame.K_RETURN:
                        if self.selected_index == 0:
                            if self.selected_ennemi_ids:
                                return {"Equipe": self.pokemons, "Ennemis possibles": [p for p in self.ennemi if p.id in self.selected_ennemi_ids]}
                    ...
                    if event.key == pygame.K_UP: self.selected_index = (self.selected_index - 1) % len(self.pokemons)
                    elif event.key == pygame.K_DOWN: self.selected_index = (self.selected_index + 1) % len(self.pokemons)
                    elif event.key == pygame.K_RETURN:
                        if self.moving_index is None: self.moving_index = self.selected_index
                        else:
                            i1, i2 = self.moving_index, self.selected_index
                            self.pokemons[i1], self.pokemons[i2] = self.pokemons[i2], self.pokemons[i1]
                            self.moving_index = None
                            save_team(self.pokemons)
                    elif event.key == pygame.K_ESCAPE: 
                        self.state = "MENU"
                        self.moving_index = None
                elif self.state == "ENNEMI":
                    if event.key == pygame.K_UP: self.selected_index = (self.selected_index - 1) % len(self.ennemi)
                    elif event.key == pygame.K_DOWN: self.selected_index = (self.selected_index + 1) % len(self.ennemi)
                    elif event.key == pygame.K_RETURN:
                        target_id = self.ennemi[self.selected_index].id
                        if target_id in self.selected_ennemi_ids: self.selected_ennemi_ids.remove(target_id)
                        else: self.selected_ennemi_ids.append(target_id)
    ...
    def draw(self):
        ...
        if self.state == "MENU":
            ...
            for relative_i, i in enumerate(range(start_index, end_index)):
                ...
                color = (50, 200, 50) if (self.state == "ENNEMI" and poke.id in self.selected_ennemi_ids) else None
                if i == self.moving_index: color = (200, 150, 0)
